...
此指令會編譯並以 `@vscode/vsce` 打包（已驗證：成功時根目錄出現 `.vsix`）。產物名稱為 `classroom-one-click-install-<version>.vsix`（例如 `classroom-one-click-install-0.0.1.vsix`；`*.vsix` 已列在 `.gitignore`，請自行發放，不必提交）。將該檔與 `samples/classroom-installs.yaml`（或本課自訂 catalog）一併交給學生。
